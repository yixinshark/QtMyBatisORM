...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Running QtMyBatisException tests..." << std::endl;
    
    TestQtMyBatisException test;
    int result = QTest::qExec(&test, argc, argv);
    
    if (result == 0) {
        std::cout << "All QtMyBatisException tests passed!" << std::endl;
    } else {
        std::cout << "Some QtMyBatisException tests failed!" << std::endl;
    }
