...
void TestExceptionIntegration::testConfigurationManagerInvalidJson()
{
    ...
    try {
        manager->loadConfiguration(m_configPath);
        QFAIL("Expected ConfigurationException for invalid JSON");
    } catch (const ConfigurationException& e) {
        QVERIFY(e.code().contains("CONFIG"));
        QVERIFY(e.getAllContext().contains("configPath"));
        QCOMPARE(e.getContext("configPath").toString(), m_configPath);
        qDebug() << "Caught expected exception:" << e.fullMessage();
    }
}
...
{
    createTestConfig(m_configPath, true);
    
    // Create two mappers with same namespace
    QString mapper1Path = m_tempDir->path() + "/mapper1.xml";
    QString mapper2Path = m_tempDir->path() + "/mapper2.xml";
    
    createTestMapper(mapper1Path, true);
    createTestMapper(mapper2Path, true);
    
    ConfigurationManager* manager = ConfigurationManager::instance();
    manager->loadConfiguration(m_configPath);
    
    try {
        manager->loadMappers({mapper1Path, mapper2Path});
        QFAIL("Expected ConfigurationException for duplicate namespace");
    } catch (const ConfigurationException& e) {
        QCOMPARE(e.code(), QString("CONFIG_DUPLICATE_NAMESPACE"));
        QVERIFY(e.getAllContext().contains("namespace"));
        QVERIFY(e.getAllContext().contains("xmlPath"));
        QVERIFY(e.getAllContext().contains("existingMapperPath"));
        qDebug() << "Caught expected exception:" << e.fullMessage();
    }
}
...
QTEST_MAIN(TestExceptionIntegration)
#include "test_exception_integration.moc"