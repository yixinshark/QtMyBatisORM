...
private slots:
    ...
    void testHandleSingleResultMultipleRows();
    void testHandleListResult();
    void testHandleListResultEmpty();
    void testHandleListResultLarge();
...
void TestResultHandler::testHandleSingleResultMultipleRows()
{
    QSqlQuery query(*m_connection);
    query.exec("SELECT * FROM test_data WHERE age IS NOT NULL ORDER BY name");
...
void TestResultHandler::testHandleListResultLarge()
{
    // 创建大量数据来测试行数限制
    QSqlQuery query(*m_connection);
    ...
    // 插入更多测试数据
    for (int i = 0; i < 100; ++i) {
        query.exec(QString("INSERT INTO test_data (name, age) VALUES ('User%1', %2)").arg(i).arg(20 + i));
    }
    
    query.exec("SELECT * FROM test_data");
    QVariantList results = m_handler->handleListResult(query);
...
void TestResultHandler::testNormalizeValue()
{
    // 由于normalizeValue是私有方法，我们通过convertFromSqlType间接测试
    
    // 测试空字符串标准化
    QVariant emptyString = m_handler->convertFromSqlType(QVariant(""));
    QVERIFY(emptyString.isNull());
    
    // 测试正常值标准化
    QVariant normalInt = m_handler->convertFromSqlType(QVariant(42));
    QCOMPARE(normalInt.toInt(), 42);
    
    QVariant normalString = m_handler->convertFromSqlType(QVariant("Hello"));
    QCOMPARE(normalString.toString(), QString("Hello"));
}

void TestResultHandler::testParseJsonValue()
{
    // 由于parseJsonValue是私有方法，我们通过convertFromSqlType间接测试
    
    // 测试有效JSON对象
    QString validJson = R"({"key": "value", "number": 123})";
    QVariant result = m_handler->convertFromSqlType(QVariant(validJson), "json");
    QVERIFY(result.canConvert<QVariantMap>());
    
    // 测试有效JSON数组
    QString validArray = R"([1, 2, 3])";
    QVariant arrayResult = m_handler->convertFromSqlType(QVariant(validArray), "json");
    QVERIFY(arrayResult.canConvert<QVariantList>());
}

void TestResultHandler::testParseJsonArray()
{
    // 由于parseJsonArray是私有方法，我们通过convertFromSqlType间接测试
    
    QString validArray = R"(["a", "b", "c"])";
    QVariant result = m_handler->convertFromSqlType(QVariant(validArray), "array");
    QVERIFY(result.canConvert<QVariantList>());
    
    QVariantList list = result.toList();
    QCOMPARE(list.size(), 3);
    QCOMPARE(list[0].toString(), QString("a"));
}
