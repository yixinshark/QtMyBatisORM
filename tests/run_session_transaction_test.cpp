...
class TestSessionTransactionStandalone : public QObject
{
    Q_OBJECT
...
void TestSessionTransactionStandalone::initTestCase()
{
    // 注册SQLite驱动
...
void TestSessionTransactionStandalone::cleanupTestCase()
{
...
void TestSessionTransactionStandalone::init()
{
    setupTestDatabase();
    createTestTable();
...
void TestSessionTransactionStandalone::cleanup()
{
    if (m_session) {
...
void TestSessionTransactionStandalone::setupTestDatabase()
{
...
void TestSessionTransactionStandalone::createTestTable()
{
...
// 基本事务测试
void TestSessionTransactionStandalone::testBasicTransactionCommit()
{
...
void TestSessionTransactionStandalone::testBasicTransactionRollback()
{
    QVERIFY(!m_session->isInTransaction());
...
void TestSessionTransactionStandalone::testTransactionWithoutBegin()
{
    QVERIFY(!m_session->isInTransaction());
...
void TestSessionTransactionStandalone::testDoubleBeginTransaction()
{
    m_session->beginTransaction();
    QVERIFY(m_session->isInTransaction());
...
// 事务超时测试
void TestSessionTransactionStandalone::testTransactionTimeout()
{
    // 开始一个1秒超时的事务
...
void TestSessionTransactionStandalone::testTransactionTimeoutCheck()
{
...
void TestSessionTransactionStandalone::testTransactionWithoutTimeout()
{
    // 开始无超时事务
...
// 嵌套事务测试 (保存点)
void TestSessionTransactionStandalone::testSavepointCreation()
{
    m_session->beginTransaction();
    QCOMPARE(m_session->getTransactionLevel(), 1);
...
void TestSessionTransactionStandalone::testSavepointRollback()
{
    m_session->beginTransaction();
...
void TestSessionTransactionStandalone::testSavepointRelease()
{
    m_session->beginTransaction();
...
void TestSessionTransactionStandalone::testMultipleSavepoints()
{
    m_session->beginTransaction();
...
void TestSessionTransactionStandalone::testSavepointOutsideTransaction()
{
    QVERIFY(!m_session->isInTransaction());
...
// 事务状态查询测试
void TestSessionTransactionStandalone::testTransactionLevel()
{
    QCOMPARE(m_session->getTransactionLevel(), 0);
...
void TestSessionTransactionStandalone::testTransactionStartTime()
{
...
void TestSessionTransactionStandalone::testTransactionTimeoutStatus()
{
    // 无超时事务
    m_session->beginTransaction();
    QVERIFY(!m_session->isTransactionTimedOut());
...
// 错误处理测试
void TestSessionTransactionStandalone::testTransactionWithClosedSession()
{
    m_session->close();
...
void TestSessionTransactionStandalone::testTransactionWithInvalidConnection()
{
    // 关闭数据库连接
    m_connection->close();
...
void TestSessionTransactionStandalone::testSavepointErrors()
{
    m_session->beginTransaction();
...
QTEST_MAIN(TestSessionTransactionStandalone)
#include "run_session_transaction_test.moc"