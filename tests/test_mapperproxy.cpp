...
void TestMapperProxy::testTemplateInvokeMethods()
{
    ...
    // 这些调用应该能够编译通过，但由于没有真实Session会抛出异常
    try {
        // 测试不同返回类型的模板方法
        QVariant result1 = proxy.invoke<QVariant>("selectById", QVariantList() << 1);
        QFAIL("Expected MappingException due to null session");
    } catch (const MappingException& e) {
        // 预期的异常，因为Session为null
        QVERIFY(QString(e.what()).contains("Session is null"));
    }
    
    try {
        QVariantList result2 = proxy.invoke<QVariantList>("selectAll");
        QFAIL("Expected MappingException due to null session");
    } catch (const MappingException& e) {
        QVERIFY(QString(e.what()).contains("Session is null"));
    }
    ...
    try {
        int result3 = proxy.invoke<int>("insertUser", QVariantList() << "John" << "john@test.com" << 30);
        QFAIL("Expected MappingException due to null session");
    } catch (const MappingException& e) {
        QVERIFY(QString(e.what()).contains("Session is null"));
    }
...
void TestMapperProxy::testInvokeNonExistentMethod()
{
    ...
    try {
        ...
        QVERIFY(QString(e.what()).contains("Statement not found"));
    }
}
