...
private slots:
    ...
    void testCreateSQLiteFactory();
    void testCreateMySQLFactory();
    void testCreateSessionFactoryDirectly();
    void testCreateSessionFactoryFromConfigFile();
...
void TestQtMyBatisORM::testCreateDefaultFactory()
{
...
void TestQtMyBatisORM::testCreateSQLiteFactory()
{
    QString dbPath = "/tmp/test.db";
    ...
    DatabaseConfig config = orm->getDatabaseConfig();
    QCOMPARE(config.driverName, QString("QSQLITE"));
    QCOMPARE(config.databaseName, dbPath);
    QCOMPARE(config.maxConnections, 10);
    QVERIFY(config.cacheEnabled);
}

void TestQtMyBatisORM::testCreateMySQLFactory()
{
    ...
    QCOMPARE(config.databaseName, QString("testdb"));
    QCOMPARE(config.userName, QString("user"));
    QCOMPARE(config.password, QString("password"));
}

void TestQtMyBatisORM::testCreateSessionFactoryDirectly()
{
    DatabaseConfig config = createValidDatabaseConfig();
    
    QSharedPointer<SessionFactory> factory = QtMyBatisORM::QtMyBatisORM::createSessionFactoryWithConfig(config);
    QVERIFY(!factory.isNull());
    QVERIFY(!factory->isClosed());
...
void TestQtMyBatisORM::testCreateSessionFactoryFromConfigFile()
{
    QString configPath = createValidConfigFile();
    
    QSharedPointer<SessionFactory> factory = QtMyBatisORM::QtMyBatisORM::createSessionFactory(configPath);
    QVERIFY(!factory.isNull());
    QVERIFY(!factory->isClosed());
...
int main(int argc, char *argv[])
{
    ...
    // 设置应用程序信息
    app.setApplicationName("QtMyBatisORM-Test");
    app.setApplicationVersion("1.0");
    
    // 检查SQLite驱动是否可用
    if (!QSqlDatabase::isDriverAvailable("QSQLITE")) {
        qCritical() << "QSQLITE driver is not available";
        return -1;
    }
    ...
    // 运行测试
    TestQtMyBatisORM test;
    int result = QTest::qExec(&test, argc, argv);
    
    return result;
}

#include "run_qtmybatisorm_test.moc"