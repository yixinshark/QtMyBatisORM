                                ...
                                const QString& statementId, bool useCache)
{
    ...
    try {
        ...
        throw SqlExecutionException(
            QStringLiteral("Unexpected error during query execution: %1").arg(QString::fromUtf8(e.what()))
        );
    }
}
                            ...
                            const QString& statementId, bool invalidateCache)
{
    ...
    try {
        ...
        throw SqlExecutionException(
            QStringLiteral("Unexpected error during update execution: %1").arg(QString::fromUtf8(e.what()))
        );
    }
}
...
QString Executor::generateCacheKey(const QString& statementId, const QVariantMap& parameters)
{
    ...
    return QStringLiteral("cache_%1_%2").arg(statementId).arg(hash, 8, 16, QChar('0'));
}
